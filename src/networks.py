...
def _conv_block(net, num_filters, filter_sizer, strides, name, trainable=True, padding="VALID", pad_mode="SYMMETRIC",
                **kwargs):
...
def resize_concat(tensor, concat_tensor, name, num_filters, trainable, **kwargs):
  ...
  deconv = _conv_block(deconv, num_filters, 3, 1, name, trainable=trainable, **kwargs)
  return deconv
...
def G(img, trainable=True, norm_name="bn1"):
  with tf.variable_scope("generator"):
    ...
    middle = _conv_block(middle, 128, 3, 1, "middle", trainable=trainable, **kwargs)  # [-1,32,32,128]

    deconv4 = resize_concat(middle, conv4, "deconv5", 128, trainable)
    deconv3 = resize_concat(deconv4, conv3, "deconv4", 128, trainable)
    deconv2 = resize_concat(deconv3, conv2, "deconv3", 64, trainable)
    deconv1 = resize_concat(deconv2, conv1, "deconv2", 32, trainable)

    uconv1 = _conv_block(deconv1, 16, 3, 1, "uconv1", trainable=trainable, **kwargs)
    uconv0 = _conv_layer(uconv1, 3, 3, 1, "uconv0", trainable=trainable)
    res = img + uconv0
...
def D(img, trainable=True):
  with tf.variable_scope("discriminator"):
    kwargs = {"nonlinearity": "lrelu", "norm_type": "instance"}
